function buildSkillGraph(path, profile) {
  ...
  const nodes = [{ id: '__role__', label: roleName, status: 'role', completed: false, color: '#a78bfa', position: [0, 0, 0] }, ...[...all.values()].map((x, i) => {
    ...
    return { id: x.name, label: x.name, status, completed: status === 'mastered', color: status === 'mastered' ? '#00d4aa' : status === 'learning' ? '#f5a623' : status === 'missing' ? '#ef4444' : '#60a5fa', position: [Math.cos(angle)*radius, Math.sin(angle)*radius*.65, (i%3-1)*.6] };
  })];
  ...
  nodes.slice(1).forEach((n) => { if (n.status !== 'mastered') connections.push({ start: roleNode.position, end: n.position, color: '#a78bfa', animated: n.status === 'recommended' }); });
  path.phases.flatMap(p => p.courses).forEach(c => {
    ...
    (c.prerequisites || []).forEach(pr => {
      ...
      if (source && target && source.id !== target.id) connections.push({ start: source.position, end: target.position, color: '#60a5fa', animated: target.status === 'recommended' });
    });
  });
...
export function transformPathResponse(path, profile = null) {
  ...
  return { id: path._id, title: path.targetRole, description: `A personalized roadmap toward becoming a ${path.targetRole}, built from your goals and current skills.`, difficulty: flatCourses[Math.floor(flatCourses.length/2)]?.level || 'intermediate', color:'#f5a623', estimatedWeeks:path.estimatedDurationWeeks, totalHours:path.estimatedDurationWeeks*8, skillGaps:path.skillGaps, nodes, skillGraph:graph };
}
