 ...
 *   - Language preference
 *   - Resource quality
 *   - Learned learner preference (warm-started from a persisted model)
 *   - MMR-style diversity
 */
...
export function matchRole(targetRoleText, roles) {
  ...
  for (const role of roles) {
    if (role.aliases.some((a) => containsAsWholePhrase(text, a) || containsAsWholePhrase(a, text))) {
      return role;
    }
  }
...
function getKnownSkillSet(currentSkills = [], knowledgeState = []) {
  ...
  // Evidence-backed mastery takes precedence over self-reported skills.
  for (const state of knowledgeState || []) {
    if (Number(state.level) >= 0.55 && Number(state.confidence) >= 0.45) {
...
export function scoreCourse(course, { requiredSkillsSet, gapSet, profile, semanticMatch = 0, learnedPreference = 0.5 }) {
  ...
  const skillGapMatch = courseSkills.length
    ? courseSkills.filter((s) => gapSet.has(s)).length / courseSkills.length
    : 0;

  const goalRelevance = courseSkills.length
    ? courseSkills.filter((s) => requiredSkillsSet.has(s)).length / courseSkills.length
    : 0;
  ...
  const languageMatch = profile.preferredLanguage && course.language === profile.preferredLanguage ? 1 : 0;
  ...
  // Learner preference is deliberately capped so relevance/safety signals
  // remain dominant — a few pieces of feedback can't override role fit.
...
function explainWhy(course, breakdown, profile, role, gaps) {
  ...
  if (breakdown.prereqReadiness >= 0.99) {
    reasons.push('Prerequisites are satisfied');
  }
  if (breakdown.quality >= 0.75) {
    reasons.push('High resource quality score');
  }
  if (breakdown.learnedPreference >= 0.65) {
    reasons.push('Matches patterns from your previous feedback');
  }
  if (profile.preferredLanguage && course.language === profile.preferredLanguage) {
    reasons.push(`Matches your ${course.language} preference`);
  }

  const known = getKnownSkillSet(profile.currentSkills, profile.knowledgeState);
  for (const p of course.prerequisites || []) {
    if (!known.has(p.toLowerCase())) {
      blockers.push(`Requires ${p}, which is not yet mastered`);
    }
  }
  if (!gapSkills.length) {
    blockers.push('Covers fewer of your current role skill gaps');
  }
  if (breakdown.goalRelevance < 0.2) {
    blockers.push('Lower relevance to your target role');
  }
 ...
 * @param {{weights:number[], bias:number}} [options.priorModel] — a
 *   previously persisted preference model (see Profile.preferenceModel) to
 *   warm-start the learner-preference layer from, instead of retraining
 *   from zero on every call.
 */
export async function rankCourses(courses, profile, role, options = {}) {
  ...
  const baseScoreBuilder = (course) => {
    const semanticMatch = semanticScores.get(String(course._id || course.id || course.title)) || 0;
    return scoreCourse(course, { requiredSkillsSet, gapSet, profile, semanticMatch, learnedPreference: 0.5 });
  };

  // Train (or incrementally update) the learner preference model from
  // historical feedback, warm-started from any previously persisted model.
  const feedbackExamples = buildFeedbackExamples(profile, filtered, baseScoreBuilder);
  const priorModel = options.priorModel;
  const preferenceModel = trainPreferenceModel(feedbackExamples, {
    initialWeights: priorModel?.weights,
    initialBias: priorModel?.bias,
  });

  const scored = filtered
    .map((course) => {
      const courseKey = String(course._id || course.id || course.title);
      const semanticMatch = semanticScores.get(courseKey) || 0;

      const isRelevant =
        course.skills.map((s) => s.toLowerCase()).some((s) => requiredSkillsSet.has(s)) ||
        semanticMatch >= 0.18;
      ...
      return {
        ...
        explanation,
        ml: {
          semanticModel: 'hybrid-tfidf + dense-embedding-cosine',
  ...
  // MMR-style diversity pass: keeps high relevance while encouraging
  // coverage of new skill gaps. Candidate pool capped for performance.
  ...
  while (pool.length) {
    ...
    for (let i = 0; i < pool.length; i++) {
      ...
      // MMR-inspired adjustment: relevance + novelty bonus - similarity penalty.
      const adjusted = item.breakdown.totalScore + 0.06 * novelty - 0.012 * overlap;
    ...
    diverse.push(chosen);
    chosen.course.skills.forEach((s) => {
      const normalized = s.toLowerCase();
      if (gapSet.has(normalized)) coveredGapSkills.add(normalized);
    });
  }

  // Return a bounded candidate set — enough alternatives remain for
  ...
  return {
    ranked: finalRanked,
    skillGaps: gaps,
    // Exposed so callers (routes) can persist it on the learner's profile
    // and pass it back in as `options.priorModel` next time — this is what
    // makes the "online" preference model genuinely persistent across
    // sessions instead of being retrained from zero every request.
    learnedModel: {
      weights: preferenceModel.weights,
      bias: preferenceModel.bias,
      trained: preferenceModel.trained,
      samples: preferenceModel.samples,
    },
    model: {
      semantic: 'Hybrid TF-IDF + dense embedding cosine + quality-aware ranking',
